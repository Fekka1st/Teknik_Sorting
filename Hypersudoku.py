...
def labelGrid():
    ...
    for row_name in rows:
        turtle.write(row_name, move=False, align='left',
                     font=('Georgia', 40, 'normal'))
        turtle.sety(turtle.ycor() - 40)

    turtle.goto(-65, 75)
    for col_name in cols:
        turtle.write(col_name, move=False, align='left',
                     font=('Georgia', 40, 'normal'))
...
def eraseEntry(puzzle):
    ...
    ask_user = turtle.textinput(
        '', 'Do you want to erase the previous entry? (y/n)')
...
def take_input(puzzle):
    ...
    user_input = turtle.textinput(
        '', 'Masukan Kolom dan baris (Contoh " B4 ")')

    x_cor = 0
    y_cor = 0

    for r in range(0, len(grid_rows)):
        # First 'element' of the user's input will be the row. E.g B1
    ...
    get_input = turtle.textinput('', 'Masukkan nomor dan buat langkah Anda!')
            ...
            for j in range(0, len(grid_cols)):
                if user_input[1] == grid_cols[j]:
                    if puzzle[i][j] != 0:  # Call the eraseEntry function and ask
                        # Ask the user if they want to erase the entry where there isn't a zero
                        eraseEntry(puzzle)
                    puzzle[i][j] = int(get_input)
                    make_move = turtle.write(
                        get_input, move=False, align='left', font=('Arial', 35, 'normal'))
    ...
    for row in range(0, len(puzzle)):  # Enter the row
        ...
        for col in range(0, len(puzzle[row])):
            if puzzle[row][col] != 0:  # If the initial base is not 0, populate the grid
                ...
                turtle.write(puzzle[row][col], move=False,
                             align='left', font=('Arial', 35, 'normal'))
...
def col_check(puzzle):
    ...
    # Turns each 'row' of the puzzle into a column. Kind of how you invert a matrice
    col_1 = [s[0] for s in puzzle]
    # so for example. col_1 = [A1, B1, C1, D1], col_2 = [A2, B2, C2, D2] and so on.
    col_2 = [s[1] for s in puzzle]
    # After the transformation is done, it applies the checker. The checker looks
    col_3 = [s[2] for s in puzzle]
    # at the length of each column. If every number is unique len(set(col_1)) = 4 and
    col_4 = [s[3] for s in puzzle]
    # if that matches with our actual column return True.
...
def box_check(puzzle):
    ...
    # so for example, box_1 = [A1,A2,B1,B2], box_2 = [A3,A4,B3,B4] and so on
...
while True:
    ...
    turtle.pensize(2)
    make_box()
    # Start by going to the left top of the grid (Labeling starts with 'A', 'B'... etc)
    turtle.goto(-95, 45)
    labelGrid()
    # Pick a random puzzle to start the game
    ...
    while game_won != True:
        ...
        if isboardfilled(game_puzzle):
            confirm_sol = turtle.textinput(
                '', 'Are you sure you want to submit this solution? (y/n)')
            if confirm_sol in ['yes', 'y', 'YES']:
                if row_check(game_puzzle) == col_check(game_puzzle) == box_check(game_puzzle) == True:
                    turtle.textinput(
                        '', 'Congratulations you have won! (Press any key to continue)')
                    game_won = True
                else:
                    turtle.textinput(
                        '', "Your solution is incorrect. Keep trying! (Press any key to continue)")
